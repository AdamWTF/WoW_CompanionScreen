...
function Controller:GetOverride(layer, control)
    local assignments = WCSDB and WCSDB.controller and WCSDB.controller.assignments
...
function Controller:GetAssignment(layer, control)
    ...
    if override then
        local metadata = WCS.SystemActions:Get(override.action)
...
function Controller:IsEnabled() return WCSDB.controller.enabled end

function Controller:EnsureCursorFrame()
    if self.cursorFrame then return end
    local frame = CreateFrame("Frame", "WCSSystemActionCursor", UIParent); frame:SetSize(42, 42); frame:SetFrameStrata("TOOLTIP"); frame:EnableMouse(false); frame:Hide()
...
function Controller:BeginSystemAction(action, originLayer, originControl)
    ...
    local metadata = WCS.SystemActions:Get(action)
...
function Controller:SetOverride(layer, control, value)
    WCSDB.controller.assignments[layer][control] = value
...
function Controller:DropOn(layer, control)
    ...
    WCSDB.controller.assignments[layer][control] = nil; PlaceAction(actionID)
    local button = self.buttons[layer] and self.buttons[layer][control]; if button then self:Apply(button) end
...
function Controller:SyncSystemActions()
    if not self.syncDirty or not WCS.Native:IsSystemActionBridgeAvailable() then return false end
    if WCS.Native:ResetSystemActions() ~= true then return false end
    for _, layer in ipairs(C.CONTROLLER_LAYERS) do
        for _, control in ipairs(C.CONTROLLER_CONTROLS) do
            ...
            if assignment then
                ...
                local supported = WCS.Native:SetSystemAction(layer, control, assignment.action)
