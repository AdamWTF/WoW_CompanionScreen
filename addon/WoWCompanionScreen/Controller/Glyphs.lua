...
function WCS.Glyphs:Get(control)
    if directional[control] then return WCS.Constants.MEDIA .. directional[control], fallback[control], 1, 1, 1 end
    local configured = WCSGamepadConfiguredGlyphStyle
    local family
    ...
    elseif configured == "Thor" then family = "aynthor"
        ...
        family = WCSDB.controller.glyphFamily
        if family == "auto" then
            local detected = WCSGamepadDetectedGlyphStyle
    ...
    if family == "playstation" and playstation[control] then
        return WCS.Constants.MEDIA .. playstation[control], fallback[control], 1, 1, 1
    end
    local glyphs = family == "aynthor" and aynthor or xbox
    if glyphs[control] then return WCS.Constants.MEDIA .. glyphs[control], fallback[control], 1, 1, 1 end
    return nil, fallback[control], .9, .9, .9
end

function WCS.Glyphs:GetModifier(modifier)
    if modifiers[modifier] then return WCS.Constants.MEDIA .. "glyphs\\" .. modifiers[modifier] end
end
