local addonName, WCS = ...
local C, Widgets = WCS.Constants, WCS.UI.Widgets

WCS.Settings = { selectedPage = 1, selectedLayer = "default" }
local Settings = WCS.Settings
...
local function showTooltip(cell, descriptor)
    ...
    elseif descriptor then GameTooltip:SetText(descriptor.name or C.CONTROLLER_CONTROL_LABELS[cell.control] or "WCS action"); GameTooltip:AddLine(descriptor.type, .7, .7, .7) end
    GameTooltip:Show()
end
...
function Settings:CreateControllerPage(parent)
    ...
    local help = page:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); help:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -8); help:SetText("Drag WoW actions or WCS System Actions onto any controller position.")
    ...
    for index, control in ipairs(C.CONTROLLER_CONTROLS) do
        ...
        cell:SetScript("OnEnter", function(button) showTooltip(button, WCS.Controller:GetAssignment(self.selectedLayer, button.control)) end); cell:SetScript("OnLeave", function() GameTooltip:Hide() end)
        cell:SetScript("OnDragStart", function(button)
            ...
            if WCS.Controller:TakeAssignment(self.selectedLayer, button.control) then self:RefreshActions() end
        end)
        cell:SetScript("OnDragStop", function() WCS.Controller:ScheduleCursorCancel() end)
        local function drop(button)
            local ok, reason = WCS.Controller:DropOn(self.selectedLayer, button.control)
        ...
        cell:SetScript("OnMouseUp", function(button, mouseButton)
            if mouseButton == "RightButton" then
                ...
                else WCS.Controller:ClearAssignment(self.selectedLayer, button.control); self:SetStatus("Controller action slot cleared.") end
                self:RefreshActions()
            elseif GetCursorInfo() or WCS.Controller:HasInternalCursor() then drop(button) end
        end)
    end
    ...
    local function createSystemActionCell(action, index)
        ...
        cell:SetScript("OnDragStart", function(button)
            local ok, reason = WCS.Controller:BeginSystemAction(button.systemAction)
            if not ok and reason == "combat" then self:SetStatus("Leave combat to edit protected controller actions.", true) end
        end)
        cell:SetScript("OnDragStop", function() WCS.Controller:ScheduleCursorCancel() end)
    end
    local systemIndex = 0; for _, action in ipairs(WCS.SystemActions:All()) do systemIndex = systemIndex + 1; createSystemActionCell(action, systemIndex) end
end
...
function Settings:CreateSecondScreenPage(parent)
    ...
    local help = page:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); help:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -8); help:SetText("These are WoW action slots 25-48, shown by WCS as slots 1-24.")
...
function Settings:CreateDisplayPage(parent)
    ...
    local function changeScale(delta) WCSDB.display.actionScale = math.max(.75, math.min(1.35, WCSDB.display.actionScale + delta)); self:RefreshAll() end
    ...
    self.regenerate:SetScript("OnClick", function() if WCS.Bridge:RegeneratePairingCode() then self:SetStatus("Pairing code regenerated.") else self:SetStatus("Native bridge is unavailable.", true) end; self:RefreshBridge() end)
end

function Settings:Create()
    local frame = CreateFrame("Frame", "WCSConfig", UIParent, "UIPanelDialogTemplate"); frame:SetSize(732, 520); frame:SetPoint("CENTER"); frame:SetFrameStrata("DIALOG"); frame:SetToplevel(true); frame:SetMovable(true); frame:EnableMouse(true); frame:RegisterForDrag("LeftButton")
    frame:SetScript("OnDragStart", function(self) self:StartMoving() end); frame:SetScript("OnDragStop", function(self) self:StopMovingOrSizing() end); frame:Hide(); frame.title:SetText("WoW Companion Screen")
    tinsert(UISpecialFrames, "WCSConfig"); self.frame, self.pages, self.navigation = frame, {}, {}
    ...
    for index, creator in ipairs(creators) do
        ...
        if not ok then
            ...
            errorText:SetText("|cffff5050WCS could not build this page:|r\n" .. tostring(message))
            if DEFAULT_CHAT_FRAME then DEFAULT_CHAT_FRAME:AddMessage("|cffff5050WCS page error:|r " .. tostring(message)) end
        end
    end
...
function Settings:RefreshDisplayPage()
    ...
    self.controllerCheck:SetChecked(WCSDB.controller.enabled); self.navigationCheck:SetChecked(WCSDB.controller.uiNavigation); self.screenCheck:SetChecked(WCSDB.secondScreen.enabled); self.reduceCheck:SetChecked(WCSDB.secondScreen.reduceUI)
    if WCSDB.controller.enabled and WCS.Native:IsUINavigationAvailable() then self.navigationCheck:Enable() else self.navigationCheck:Disable() end
    self.controllerNative:SetText(WCS.Native:IsControllerAvailable() and "|cff55ff55Native controller integration available|r" or "|cff888888Native controller integration unavailable|r")
    if WCSDB.controller.enabled and WCSDB.secondScreen.enabled then self.reduceCheck:Enable() else self.reduceCheck:Disable() end
...
function Settings:RefreshBridge()
    ...
    local state = WCS.Bridge:GetStatus()
...
function Settings:RefreshActions() self:RefreshControllerPage(); self:RefreshSecondScreenPage(); WCS.ActionOverlay:Refresh() end
