...
function Widgets:SetGlyph(cell, control)
    local texture, label, red, green, blue = WCS.Glyphs:Get(control)
    ...
    cell.glyphFrame:SetScale((WCSDB.display.glyphScale or 1) * .75)
end
...
function Widgets:RefreshControllerCell(cell)
    local descriptor = WCS.Controller:GetAssignment(cell.layer, cell.control); local state = WCS.Controller:GetState(descriptor); self:SetGlyph(cell, cell.control)
