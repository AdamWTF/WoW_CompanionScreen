...
ThorPad.UINavigation = { candidates = {}, roots = {}, active = false, elapsed = 0 }
local Navigation = ThorPad.UINavigation
...
local function interactive(frame)
    if not frame or frame.ThorPadIgnoreNavigation or not frame.IsVisible or not frame:IsVisible() or not enabled(frame) then return false end
...
function Navigation:GetVisibleRoots()
    ...
    addRoot(roots, named("MerchantFrame"), "merchant", 50); addRoot(roots, named("ThorPadConfig"), "thorpad", 45)
...
function Navigation:ChooseDefault()
    ...
    elseif root.kind == "thorpad" and ThorPad.Settings and ThorPad.Settings.navigation then
        local candidate = self:CandidateFor(ThorPad.Settings.navigation[1]); if candidate then return candidate end
    end
...
function Navigation:EnsureHighlight()
    ...
    local frame = CreateFrame("Frame", "ThorPadUINavigationFocus", UIParent); frame:SetFrameStrata("TOOLTIP"); frame:EnableMouse(false); frame:Hide()
...
function Navigation:SetSelected(candidate, movePointer)
    ...
    if movePointer then
        ...
        if width > 0 and height > 0 then ThorPad.Native:MoveUINavigationPointer(candidate.x / width, candidate.y / height) end
    end
end
...
function Navigation:SetActive(active)
    ...
    if not ThorPad.Native:SetUINavigationActive(active) and active then active = false end
    self.active = active
...
function Navigation:ShouldActivate()
    return ThorPadDB and ThorPadDB.controller and ThorPadDB.controller.enabled and ThorPadDB.controller.uiNavigation and not InCombatLockdown() and ThorPad.Native:IsUINavigationAvailable() and #self.roots > 0 and #self.candidates > 0
end
...
function Navigation:Handle(command)
    ...
    if command == "confirm" then
        ...
        self:SetSelected(current, true); ThorPad.Native:ClickUINavigationPointer(current.kind == "bag_item" and "right" or "left"); return
    end
