...
{
    WcsBridge* g_bridge = nullptr;
    namespace
    {
        constexpr const char* kConfig = "Extensions\\wcs-bridge\\wcs-bridge.cfg";
        ...
        uint16_t ConfigPort()
        {
            const std::string value = ConfigString("WCS_BRIDGE_PORT", "18423"); char* end = nullptr; const unsigned long port = std::strtoul(value.c_str(), &end, 10);
    ...
    bool WcsBridge::Initialise()
    {
        enabled_ = ConfigBool("WCS_BRIDGE_ENABLED", true); bindAddress_ = ConfigString("WCS_BRIDGE_BIND_ADDRESS", "0.0.0.0"); port_ = ConfigPort();
        if (!pairing_.Initialise(ConfigBool("WCS_BRIDGE_REQUIRE_PAIRING", true))) { Log(WXL_LOG_ERROR, "pairing manager could not initialise"); return false; }
        ...
        else if (enabled_) { char line[192]; std::snprintf(line, sizeof line, "starting ws://%s:%u/wcs", bindAddress_.c_str(), unsigned(port_)); Log(WXL_LOG_INFO, line); }
        else Log(WXL_LOG_INFO, "disabled by configuration; Lua/status integration remains available");
        return true;
    }

    bool WcsBridge::Enqueue(Command command)
    {
    ...
    void WcsBridge::OnUpdate(const wxl::events::UpdateArgs&)
    {
    ...
    bool WcsBridge::PublishSnapshot(std::string_view payload, std::string& error)
    {
    ...
    bool WcsBridge::PublishEvent(std::string_view type, std::string_view payload, std::string& error)
    {
    ...
    void WcsBridge::ForgetDevice() { pairing_.Forget(); if (server_) server_->DisconnectClient(); Log(WXL_LOG_INFO, "paired device forgotten and active session revoked"); }
    void WcsBridge::SendError(std::string code) { if (server_) server_->Send(ErrorMessage(std::move(code))); }

    void WcsBridge::DrawPanel()
    {
        g_api->UiText("WoW Companion Screen bridge / WebSocket protocol v1"); g_api->UiSeparator();
        char line[256]; std::snprintf(line, sizeof line, "Endpoint: ws://%s:%u/wcs", bindAddress_.c_str(), unsigned(port_)); g_api->UiText(line);
