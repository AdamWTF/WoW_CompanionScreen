...
namespace wcs_gamepad
{
    ...
    void ControllerGameplay::Dispatch(const CompanionAction& action, InputState state, uint32_t time)
    {
        if (action.type == CompanionActionType::WoWAction)
        {
            if (state == InputState::Pressed) input_.WoWAction(action.wowActionSlot);
            return;
        }
        if (action.type != CompanionActionType::SystemAction || action.systemAction == CompanionSystemAction::Unknown) return;
        if (action.systemAction == CompanionSystemAction::Jump)
        {
        ...
        else if (action.systemAction == CompanionSystemAction::Interact && state == InputState::Pressed)
    ...
    void ControllerGameplay::Publish(const ControllerSnapshot& snapshot)
    {
        ...
        std::snprintf(script, sizeof script, "WCSGamepadNativeLayer=%d;WCSGamepadNativeConnected=%s;WCSGamepadNativeBackend='%s';WCSGamepadNativeDevice='%s';WCSGamepadDetectedGlyphStyle='%s';WCSGamepadConfiguredGlyphStyle='%s';WCSGamepadIsAvailable=function() return true end", layer_ + 1, snapshot.connected ? "true" : "false", backend.c_str(), device.c_str(), hint.c_str(), configured.c_str()); wxl::game::script::Execute(script);
    }
    ...
    void ControllerGameplay::Update(const ControllerSnapshot& snapshot, float dt, uint32_t time)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            if (buttons[i] && !actions_[i])
            {
                ...
                if (config_.debug) g_api->Log(WXL_LOG_INFO, kTag, "control=%d DOWN layer=%d slot=%d action=%s:%s", i, layer_ + 1, lastAction_, capturedActions_[i].type == CompanionActionType::SystemAction ? "SYSTEM" : "WOW", capturedActions_[i].type == CompanionActionType::SystemAction ? SystemActionName(capturedActions_[i].systemAction) : "ACTION");
            }
            else if (!buttons[i] && actions_[i])
            {
                const CompanionAction released = capturedActions_[i]; const int releasedSlot = capturedSlots_[i]; Dispatch(released, InputState::Released, time); capturedActions_[i] = {}; capturedSlots_[i] = 0;
                if (config_.debug) g_api->Log(WXL_LOG_INFO, kTag, "control=%d UP slot=%d action=%s:%s", i, releasedSlot, released.type == CompanionActionType::SystemAction ? "SYSTEM" : "WOW", released.type == CompanionActionType::SystemAction ? SystemActionName(released.systemAction) : "ACTION");
            }
