...
namespace wcs_gamepad
{
    ...
    {
        if (!api_.Initialize()) return false; api_.AddMappings("Extensions\\wcs-gamepad\\gamecontrollerdb.txt"); api_.Pump(); int count{}; auto ids = api_.Gamepads(count); g_api->Log(WXL_LOG_INFO, kTag, "[SDL] mapped gamepads detected: %d", count);
    ...
    {
        if (!api_.Initialize()) return false; api_.AddMappings("Extensions\\wcs-gamepad\\gamecontrollerdb.txt"); api_.Pump(); int count{}; auto ids = api_.Joysticks(count); g_api->Log(WXL_LOG_INFO, kTag, "[SDL] Joysticks detected: %d", count);
