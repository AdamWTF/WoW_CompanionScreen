...
- Select (Xbox View/Back, PlayStation Share/Create, Thor Select): toggle the world map.
...
- Supported WCS UI panels: D-pad spatial navigation, with configurable South/East confirm and back. Gameplay movement, camera, targeting, and actions are suppressed until the panel closes or combat begins.
...
The WCS addon synchronizes every logical action through the extension's in-process Lua API. The Default layer resolves WoW's live primary action page, including stance and form bonus pages; modifier layers remain fixed. `JUMP` uses WoW's native Jump input-control bit on press and release, including swimming/flying ascent; it never synthesizes Space. `INTERACT` runs Smart Interact once per press: it preserves positively identified interactable targets, recognizes lootable and skinnable corpses, suppresses automatic replacement in combat, and otherwise scores conservative NPC/GameObject candidates by camera alignment, distance, and verified interaction metadata. Unsupported System Action IDs are retained as inert mappings and logged rather than falling through to a WoW action.
